...
public class ComponentScanBeanDefinitionParser implements BeanDefinitionParser {
	...
	/**
	 * <context:component-scan> 节点解析已经完成，主要做的事情有：
	 *
	 * 扫描 base-package 目录，将使用了 @Component、@Controller、@Repository、@Service 注解的 bean 注册到注册表中
	 * （其实就是beanDefinitionMap、beanDefinitionNames、aliasMap缓存中），跟之前解析默认命名空间一样，也是在后续创建 bean 时需要使用这些缓存。
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		// 3.解析base-package（允许通过 ",; \t\n" 中的任一符号填写多个），例如: com.spring.demo.one;com.spring.demo.two
	...
	protected ClassPathBeanDefinitionScanner configureScanner(ParserContext parserContext, Element element) {
		...
		// 6.解析类型过滤器
		//我们知道，当我们配置了 component-scan 时，Spring会去扫描 base-package 下所有使用了
		// @Component（包括@Controller、@Repository、@Service） 注解的 bean。
		// 这是因为 use-default-filters 属性默认值为 true，
		// 而通过createScanner方法我们知道，use-default-filters = true 时，includeFilters 会有两个 AnnotationTypeFilter，
	...
	protected void parseTypeFilters(Element element, ClassPathBeanDefinitionScanner scanner, ParserContext parserContext) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				// 拿到节点的localName
				// 例如节点：<context:exclude-filter type="" expression=""/>，localName为：exclude-filte
